...
/**
 * @brief overlap
 * @details Function to compute the overlapped data between coordinate x with
 * size w
 ...
 * @details Apply Non-Maximum Suppression (NMS) to get rid of overlapped
 * rectangles.
...
void filter_boxes_nms(std::vector<Detection>& det, int32_t size, float th_nms) {
    ...
    for (i = 0; i < count; i++) {
        ...
        for (j = 0; j < count; j++) {
            ...
            if (det[i].class_id != det[j].class_id) {
                continue;
            }
            b = det[j].box;
            b_intersection = box_intersection(a, b);
            if ((box_iou(a, b) > th_nms) || (b_intersection >= a.h * a.w - 1) ||

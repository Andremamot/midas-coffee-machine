 ...
 * @brief API publik utama mod_volume — estimasi volume gelas dari raw fisheye
 * frame.
 *
 ...
 *   raw_frame → [moil_facade] → [measure_rim_width_px] → [calc_diameter] →
 * [calc_volume]
 *
 ...
 *
 * Semua nilai kecuali frame harus sudah tersedia dari pipeline
 * YOLO/ArUco/MiDaS.
 */
struct VolumeInput {
...
public:
  /**
   * @brief Konstruktor — inisialisasi moil_facade dengan params hardcoded.
   *
   * @param camera_params_json  Path ke camera_parameters.json Moildev
   * @param camera_name         Nama profil kamera di JSON (mis.
   * "syue_7730v1_6")
   ...
   */
  explicit CupVolumeEstimator(const std::string &camera_params_json,
                              const std::string &camera_name = "syue_7730v1_6",
                              int frame_width = 2592, int frame_height = 1944);
...
private:
  bool ready_ = false;
  std::unique_ptr<moil_facade> moil_;
  int frame_w_;
  int frame_h_;
};

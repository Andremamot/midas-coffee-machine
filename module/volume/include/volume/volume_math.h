/**
 * @file volume_math.h
 * @brief Fungsi estimasi volume gelas dari data frame + ArUco.
 *
 * Include file ini di proyek yang membutuhkan estimasi volume:
 *   #include <volume/volume_math.h>
 *
 * Pipeline:
 *   measureRimWidthPx() → calcDiameter() → calcVolume()
 *
...
#include <volume/height_math.h>   // untuk BBox
 ...
 * lalu cari kolom kiri-kanan yang ada piksel aktif.
 * Lebih akurat dari bbox_w penuh karena tidak terpengaruh body yang lebih lebar.
 *
 * @param frame  Frame BGR (sudah di-undistort)
 ...
 *
 * @param rim_w_px  Lebar rim dalam piksel (dari measure_rim_width_px)

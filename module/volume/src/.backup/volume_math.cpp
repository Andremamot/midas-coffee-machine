...
double measure_rim_width_px(const cv::Mat& frame, const BBox& bbox) {
    ...
    // Konversi ke grayscale (support BGR, BGRA, grayscale)
    cv::Mat gray;
    if (rim_strip.channels() == 3)
        cv::cvtColor(rim_strip, gray, cv::COLOR_BGR2GRAY);
    else if (rim_strip.channels() == 4)
        cv::cvtColor(rim_strip, gray, cv::COLOR_BGRA2GRAY);
    else
        gray = rim_strip.clone();

    // Otsu threshold
    cv::Mat mask;
    cv::threshold(gray, mask, 0, 255, cv::THRESH_BINARY | cv::THRESH_OTSU);

    /* Proyeksikan mask ke sumbu kolom dengan REDUCE_MAX — O(W×H) single-pass.
     * Jauh lebih efisien dari nested loop O(H) × W sebelumnya. */
    cv::Mat col_max;
